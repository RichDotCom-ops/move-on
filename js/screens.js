/* =========================================================
   SCREEN RENDERING FUNCTIONS - WITH GAMIFICATION & THERAPIST
========================================================= */

// Navigation
const screenIds = ['home','nocontact','journal','urge','progress','achievements','therapist','profile'];
...
function showChallengePopup() {
  ...
  overlay.style.cssText = 'position:fixed; inset:0; background:rgba(0,0,0,0.85); z-index:200; display:flex; align-items:center; justify-content:center; animation:fadein 0.3s ease;';
  overlay.innerHTML = `
    <div style="background:#0d0d0d; border:1px solid rgba(255,255,255,0.1); border-radius:24px; padding:30px 24px; margin:20px; text-align:center; max-width:320px; width:100%;">
      ...
      <button onclick="this.parentElement.parentElement.remove(); haptic('success'); toast('Challenge accepted! 🎯', 'star');" style="width:100%; padding:16px; border-radius:14px; border:none; background:#fff; color:#000; font-size:16px; font-weight:700; cursor:pointer; margin-bottom:8px;">I'll do it! 💪</button>
      <button onclick="this.parentElement.parentElement.remove();" style="width:100%; padding:12px; border-radius:14px; border:1px solid rgba(255,255,255,0.1); background:transparent; color:#666; font-size:13px; cursor:pointer;">Maybe later</button>
    </div>`;
...
function renderHome() {
  ...
  if (state.missions.length === 0) {
    ...
    state.missions.forEach((m, i) => {
      ...
      deleteBg.style.cssText = 'position:absolute; right:0; top:0; bottom:0; width:80px; background:var(--red); display:flex; align-items:center; justify-content:center; cursor:pointer; z-index:0;';
      deleteBg.innerHTML = '<svg width="20" height="20" viewBox="0 0 24 24" fill="none" stroke="white" stroke-width="2.5" stroke-linecap="round" stroke-linejoin="round"><path d="M3 6h18"/><path d="M19 6v14a2 2 0 01-2 2H7a2 2 0 01-2-2V6"/><path d="M8 6V4a2 2 0 012-2h4a2 2 0 012 2v2"/></svg>';
      
      const inner = document.createElement('div');
      inner.style.cssText = 'display:flex; align-items:center; gap:12px; width:100%; padding:11px 4px; transition:transform 0.3s ease; position:relative; z-index:1; background:var(--bg-elev); cursor:pointer; user-select:none; -webkit-user-select:none;';
      inner.innerHTML = `<div class="check ${m.done ? 'done' : ''}" style="flex-shrink:0;">${m.done ? CHECK_SVG : ''}</div><div class="mission-text" style="flex:1;">${m.text}</div>`;
      
      let startX = 0, currentX = 0, isDragging = false, isOpen = false;
      const deleteWidth = 80;
      
      inner.addEventListener('touchstart', (e) => { startX = e.touches[0].clientX; isDragging = true; inner.style.transition = 'none'; });
...
function showRewardPopup(days, reward) {
  ...
  overlay.innerHTML = `
    <div style="background:#0d0d0d; border:1px solid rgba(255,255,255,0.1); border-radius:24px; padding:30px 24px; margin:20px; text-align:center; max-width:320px; width:100%;">
      ...
      <button onclick="this.parentElement.parentElement.remove(); haptic('success');" style="width:100%; padding:16px; border-radius:14px; border:none; background:#fff; color:#000; font-size:16px; font-weight:700; cursor:pointer; margin-top:24px;">Let's go! 🚀</button>
    </div>`;
  document.body.appendChild(overlay);
  haptic('success');
}
...
function showUndoToast(mission, wasDone) {
  const t = document.getElementById('toast');
  t.innerHTML = `<span class="toast-row"><span>Mission deleted</span><button onclick="undoDelete(${mission.id}, '${mission.text.replace(/'/g, "\\'")}', ${wasDone}); event.stopPropagation();" style="background:#fff; color:#000; border:none; padding:6px 12px; border-radius:8px; font-weight:700; font-size:12px; cursor:pointer; margin-left:8px;">Undo</button></span>`;
  t.classList.add('show');
  clearTimeout(toastTimer);
...
function spawnHomeConfetti() {
  ...
  for (let i = 0; i < 30; i++) {
    ...
    dot.style.cssText = `position:fixed; width:${6+Math.random()*8}px; height:${6+Math.random()*8}px; background:${colors[Math.floor(Math.random()*colors.length)]}; border-radius:${Math.random()>0.5?'50%':'2px'}; left:${Math.random()*100}%; top:-20px; z-index:200; pointer-events:none; animation:confettiFall ${fallDuration}s ease-out forwards; animation-delay:${Math.random()*0.5}s;`;
...
function shareStreak() {
  ...
  const days = noContactDaysNow();
  const firstName = getFirstName();
  const messages = [`Day ${days} of No Contact. Not just moving on — moving up. 🚀 #MoveOnApp`,`${days} days of choosing myself. Best decision ever. 💜 #MoveOnApp`,`They said I couldn't do it. Day ${days}. Watch me. 👑 #MoveOnApp`];
...
function renderCalendar() {
  ...
  for (let i = firstDay - 1; i >= 0; i--) { const el = document.createElement('div'); el.className = 'cal-day otherm'; el.textContent = prevDaysInMonth - i; grid.appendChild(el); }
  ...
  for (let i = 1; i <= trailing; i++) { const el = document.createElement('div'); el.className = 'cal-day otherm'; el.textContent = i; grid.appendChild(el); }
}
...
function renderMoodChart() {
  const points = [];
  for (let i = 6; i >= 0; i--) { const d = new Date(today.getTime() - i*24*60*60*1000); const k = dkey(d); const entry = state.journal[k]; points.push({label: d.getDate(), mood: entry && entry.mood ? entry.mood : null}); }
...
function renderMilestones(days) {
  const defs = [{d:7,label:'7 Days',sub:'First week complete!'},{d:14,label:'14 Days',sub:'Two weeks strong!'},{d:21,label:'21 Days',sub:'New habit forming!'},{d:30,label:'30 Days',sub:"You're a champion!"},{d:60,label:'60 Days',sub:'Unstoppable!'},{d:90,label:'90 Days',sub:'New you unlocked!'}];
...
function renderAchievements() {
  ...
  const firstName = getFirstName();
  if (days === 0) { document.getElementById('achTitle').textContent = 'Begin Your Journey'; document.getElementById('achSub').innerHTML = 'Start your No Contact streak<br>to unlock achievements.'; }
  else { document.getElementById('achTitle').textContent = firstName ? `${firstName}, No Contact Champion` : 'No Contact Champion'; document.getElementById('achSub').innerHTML = firstName ? `You're building a better<br>future for yourself, ${firstName}.` : "You're building a better<br>future for yourself."; }
...
function spawnConfetti() {
  ...
  for (let i = 0; i < 16; i++) { const dot = document.createElement('div'); dot.className = 'confetti-dot'; const size = 4+Math.random()*5; dot.style.width=size+'px'; dot.style.height=size+'px'; dot.style.background=colors[i%colors.length]; dot.style.left=(10+Math.random()*80)+'%'; dot.style.top=(Math.random()*60)+'px'; wrap.appendChild(dot); }
}
